...
**当前快照里装的是真实数据集（Olist 巴西电商公开数据集）跑出来的结果：**

| 指标 | 值 |
| --- | --- |
| 订单明细（DWD） | **102,425 行** |
| 唯一订单 / 用户 / 商品 | 98,666 / 95,420 / 32,951 |
| GMV（有效订单） | **13,494,400.74 BRL** |
| 客单价 | 137.42 BRL |
| 时间跨度 | 616 天（2016-09 ~ 2018-10） |
| 数据质量得分 | 1.0000（该公开数据集已由发布方清洗过） |
| 接口数量 | 15 个 |
| 作业耗时（DataFrame / RDD） | 约 20 秒 / 约 19 秒 |

如果导入快照后查到的 `dwd_order_detail` 行数不是 **102,425**，说明导入有问题，请回看第一节。

> **金额单位是巴西雷亚尔（BRL）**，不是人民币 —— 因为用的是巴西电商数据集。
> 项目通过 `spark-job.properties` 的 `data.currency.unit` 与 `application.yml` 的
> `app.currency.unit` 控制单位，切换数据源时两处要一起改。

### 两套数据源

项目支持两套数据源，**切换不需要改代码**，只改 `data.raw.dir`：

| | 真实数据集（当前使用） | 模拟数据集（对照） |
| --- | --- | --- |
| 内容 | Olist 巴西电商 10 万笔真实订单 | 程序生成 20 万行 + 8 类脏数据 |
| 货币 | BRL | 元 |
| 数据质量得分 | 1.0000 | 0.9645 |
| 用来验证 | 真实业务口径与多维分析 | 清洗规则与数据质量治理能力 |
| 怎么获取 | `scripts\download-real-data.ps1` 后 `--import-real` | `scripts\run-etl.cmd --generate` |

详细映射与口径说明见 `spark-sales-analysis/docs/02-数据来源与数据字典.md`。
